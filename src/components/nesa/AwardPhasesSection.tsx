      ...
      <div className="container">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {phases.map((phase) => (
            <div
              key={phase.name}
              className="bg-nesa-navy-dark/50 rounded-2xl p-6 border border-nesa-gold/10 hover:border-nesa-gold/30 transition-colors"
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  <p className="text-nesa-gold text-sm font-medium">Phase {phase.number}</p>
                  <h3 className="text-xl font-bold text-white">{phase.name}</h3>
                  <p className="text-nesa-text-muted text-sm">{phase.subtitle}</p>
                </div>
              </div>

              <div className="flex flex-wrap gap-4 mb-4 text-sm">
                <span className="text-nesa-text-muted">
                  <span className="text-nesa-gold">Period:</span> {phase.period}
                </span>
                <span className="flex items-center gap-1 text-nesa-text-muted">
                  <Tv className="h-3.5 w-3.5 text-nesa-gold" />
                  <span className="text-nesa-gold">Show:</span> {phase.showDate}
                </span>
              </div>

              <ul className="space-y-2">
                {phase.bullets.map((bullet) => (
                  <li key={bullet} className="flex items-start gap-2 text-sm text-nesa-text">
                    <Check className="h-4 w-4 text-nesa-gold flex-shrink-0 mt-0.5" />
                    {bullet}
                  </li>
                ))}
              </ul>
