...
const TIERS: { slug: string; label: string; blurb: string }[] = [
  ...
  },
  {
    ...
    blurb: "Elite institutional contribution, jury-verified.",
  },
  {
...
export default function AwardLandingPagesIndex() {
      ...
      <div className="min-h-screen bg-charcoal text-white">
        <main className="mx-auto max-w-6xl px-4 py-10 md:py-14">
          ...
          {TIERS.filter((t) => active === "all" || t.slug === active).map((tier) => {
            ...
            return (
              <section key={tier.slug} className="mb-12" aria-label={`${tier.label} pages`}>
                ...
                <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                  {items.map((page) => (
                    <Link
                      key={page.slug}
                      to={`/recognition/subpage/${page.slug}`}
