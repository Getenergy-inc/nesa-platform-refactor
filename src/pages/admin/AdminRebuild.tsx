...
export default function AdminRebuild() {
  ...
  const [stageConfig, setStageConfig] = useState<
    { id: string; action: string; is_open: boolean }[]
  >([]);
  ...
  async function fetchData() {
    ...
    const [nomRes, schoolRes, winnerRes, stageRes] = await Promise.all([
      ...
      supabase
        .from("rebuild_schools")
        .select("*")
        .order("created_at", { ascending: false }),
      supabase
        .from("rebuild_winners")
        .select("*")
        .order("created_at", { ascending: false }),
      supabase
        .from("stage_config")
        .select("*")
        .in("action", ["rebuild_nominations", "rebuild_voting"]),
    ]);
    ...
    setStageConfig(
      (stageRes.data as { id: string; action: string; is_open: boolean }[]) ||
        [],
    );
    setLoading(false);
  }
  ...
  async function updateNominationStatus(
    id: string,
    status: string,
    notes?: string,
  ) {
  ...
  const filteredNominations = nominations.filter((n) => {
    ...
    const matchesSearch =
      !searchTerm ||
      n.school_name.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const stats = {
    ...
    verifiedSchools: schools.filter((s) => s.verification_status === "verified")
      .length,
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-xs text-muted-foreground">
              {stats.verifiedSchools} verified
            </p>
          </CardContent>
        </Card>
        <Card>
      ...
      <Tabs defaultValue="stages" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="nominations">
            <School className="h-4 w-4 mr-1" /> Nominations
          </TabsTrigger>
          <TabsTrigger value="schools">
            <MapPin className="h-4 w-4 mr-1" /> Schools
          </TabsTrigger>
          <TabsTrigger value="winners">
            <Trophy className="h-4 w-4 mr-1" /> Winners
          </TabsTrigger>
          <TabsTrigger value="interventions">
            <BarChart3 className="h-4 w-4 mr-1" /> Interventions
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="stages">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Open or close Rebuild campaign stages
              </CardDescription>
            </CardHeader>
              ...
              {stageConfig.map((stage) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-3 h-3 rounded-full ${stage.action === "rebuild_nominations" ? "bg-emerald-500" : "bg-amber-500"}`}
                    />
                    <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Switch
                      ...
                      onCheckedChange={() =>
                        toggleStage(stage.id, stage.is_open)
                      }
                    />
                  </div>
                </div>
              ...
              <div className="space-y-3">
                {filteredNominations.map((nom) => (
                  <NominationRow
                    ...
                    onShortlist={() =>
                      updateNominationStatus(nom.id, "shortlisted")
                    }
                  />
                ))}
                ...
                {schools.map((school) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {school.school_type} • {school.country}
                      </p>
                      ...
                      {school.verification_status === "pending" && (
                        <>
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              updateSchoolVerification(school.id, "verified")
                            }
                          >
                          ...
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              updateSchoolVerification(school.id, "rejected")
                            }
                          >
        ...
        <TabsContent value="winners">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Publish winners and manage selections
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
        ...
        <TabsContent value="interventions">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Track rebuilding progress per school
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                  ...
                  <div key={w.id} className="p-4 border rounded-lg space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Select
                        ...
                        onValueChange={(val) =>
                          updateInterventionStatus(w.id, val)
                        }
                      >
